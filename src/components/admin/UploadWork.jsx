...
export default function UploadWork() {
  ...
  const [isFileTemplate, setIsFileTemplate] = useState(false);
  ...
  const handleCheckboxChange = (event) => {
    setIsFileTemplate(event.target.checked);
  };
  ...
  const handleSubmitForm = async (event) => {
    ...
    const courseData = {
      ...
      template: isFileTemplate,
    };

    try {
      const createCourseResponse = await axios.post(
        "http://localhost:3000/courses/create-courses?typeId=3",
        courseData,
        config
      );
      ...
      console.log("Update course response:", updateCourseResponse.data.course);
